...
describe("Integration: CAP -> SEGW MPC V4 generation", () => {
	test("Application service generates V4 MPC metadata", () => {
		...
		expect(mpc).toContain("primitive_property->set_edm_type( /iwbep/if_v4_med_element=>gcs_edm_data_types-boolean ).");
		expect(mpc).toContain("primitive_property->set_edm_name( 'availableOn' ).");
		expect(mpc).toContain("primitive_property->set_edm_type( /iwbep/if_v4_med_element=>gcs_edm_data_types-date ).");
		expect(mpc).toContain("primitive_property->set_edm_name( 'changedAt' ).");
		expect(mpc).toContain("primitive_property->set_edm_type( /iwbep/if_v4_med_element=>gcs_edm_data_types-datetimeoffset ).");
